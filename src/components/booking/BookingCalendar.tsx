...
const localeByLanguage: Record<string, string> = { es: "es-ES", ca: "ca-ES", en: "en-GB" };
function activeLocale() {
  return localeByLanguage[i18n.language] ?? "es-ES";
}
...
export function BookingCalendar() {
  ...
  const serviceLabels: Record<BookingService, string> = {
    plan_329: t("services.planTitle") + " (329 €)",
    sesion_80: t("services.sessionTitle") + " (80 €)",
  };
  ...
  if (confirmedBookingId && selectedSlot) {
    return (
      <div className="card text-center">
        <h3 className="font-display text-xl font-bold text-brand-900">{t("booking.confirmedTitle")}</h3>
        <p className="mt-2 text-content-muted">
          {t("booking.confirmedOn", {
            service: serviceLabels[service],
            date: new Date(selectedSlot.slot_start).toLocaleString(activeLocale(), {
              dateStyle: "full",
              timeStyle: "short",
            }),
          })}
        </p>
        <p className="mt-2 text-sm text-content-muted">{t("booking.confirmationEmailSent", { email })}</p>
        {service === "sesion_80" && (
          <div className="mt-4 space-y-2">
            <p className="text-sm text-content-muted">{t("booking.payLaterNote")}</p>
            <button type="button" className="btn-secondary" onClick={handlePayNow} disabled={createCheckout.isPending}>
              {createCheckout.isPending ? t("booking.redirecting") : t("booking.payNow")}
            </button>
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="label">{t("booking.day")}</label>
        <div className="flex gap-2 overflow-x-auto pb-2">
          {days.map((day) => {
            ...
            return (
              ...
              >
                <span className="text-xs uppercase">{day.toLocaleDateString(activeLocale(), { weekday: "short" })}</span>
      ...
      <div>
        <label className="label">{t("booking.hour")}</label>
        {isLoading ? (
          <p className="text-sm text-content-muted">{t("booking.loadingSlots")}</p>
        ) : slots.length === 0 ? (
          <p className="text-sm text-content-muted">{t("booking.noSlots")}</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {slots.map((slot) => {
              ...
              return (
                ...
                >
                  {new Date(slot.slot_start).toLocaleTimeString(activeLocale(), { hour: "2-digit", minute: "2-digit" })}
                </button>
              );
            })}
          </div>
      ...
      {selectedSlot && (
        <div className="card space-y-4">
          ...
          {createBooking.isError && <p className="text-sm text-red-600">{t("booking.confirmError")}</p>}
        </div>
      )}
    </div>
  );
